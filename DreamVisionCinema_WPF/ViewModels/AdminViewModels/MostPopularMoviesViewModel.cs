...
{
    public class MostPopularMoviesViewModel : BaseViewModel
    {
        private static MostPopularMoviesViewModel _instance = null;

        private PlotModel? chartModel;
        public ICommand BackFromPopularMoviesCommand { get; set; }

        public MostPopularMoviesViewModel(IReservationRepository reservationRepository)
        {
            Dictionary<string, int> data = reservationRepository.GetMostPopularMovies();
            PrepareChart(data);
            BackFromPopularMoviesCommand = new RelayCommand(OpenMainStatisticPanel);
        }
        ...
        public static MostPopularMoviesViewModel Instance
        {
            get
            {
                return DIContainer.GetContainer().Resolve<MostPopularMoviesViewModel>();
                /*if (_instance == null)
                {
                    _instance = DIContainer.GetContainer().Resolve<MostPopularMoviesViewModel>();
                    return _instance;
                }
                else
                    return _instance;*/
